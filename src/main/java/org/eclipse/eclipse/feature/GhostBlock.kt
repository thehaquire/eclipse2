...
class GhostBlock {
    companion object {
        val GHOST_KEY = KeyBinding("Ghost Block", Keyboard.KEY_G, "Eclipse")
        var lastGhostBlock: Long = 0
    }
    @SubscribeEvent
    fun onRenderWorld(event: RenderWorldLastEvent?) {
        if (Handle.isModuleEnabled("SkyBlock", "Ghost Block")) {
            ...
            if (w.getBlockState(pos).block == Blocks.chest || w.getBlockState(pos).block == Blocks.trapped_chest || w.getBlockState(
                    pos
                ).block == Blocks.ender_chest || w.getBlockState(pos).block == Blocks.lever || w.getBlockState(pos).block == Blocks.stone_button || w.getBlockState(
                    pos
