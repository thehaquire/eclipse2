package org.eclipse.eclipse.theme;
 ...
 * @author lukflug
 */
public class EclipseTheme extends ThemeBase {
    /**
     * The font size.
     */
     ...
     */
    public EclipseTheme (IColorScheme scheme, int height, int padding) {
        super(scheme);
        this.height=height;
    ...
    public IContainerRenderer getContainerRenderer (int logicalLevel, int graphicalLevel, boolean horizontal) {
        return new IContainerRenderer() {
            ...
            public void renderBackground (Context context, boolean focus) {
                if (graphicalLevel==0) org.eclipse.eclipse.theme.EclipseTheme.this.renderBackground(context,focus);
            }
    ...
    public <T> IPanelRenderer<T> getPanelRenderer (Class<T> type, int logicalLevel, int graphicalLevel) {
        return new IPanelRenderer<T>() {
            ...
            @Override
            public void renderTitleOverlay (Context context, boolean focus, T state, boolean open) {
                // Keep the panel outline rendering if necessary
                if (graphicalLevel<=0) {
                    Color colorA=scheme.getColor("Panel Outline Color");
                    context.getInterface().fillRect(new Rectangle(context.getPos().x,context.getPos().y+context.getSize().height,context.getSize().width,1),colorA,colorA,colorA,colorA);
                } else {
                ...
                // Removed the rendering of the small button with the arrow
            }
        };
    }
    ...
    public <T> IButtonRenderer<T> getButtonRenderer(Class<T> type, int logicalLevel, int graphicalLevel, boolean container) {
        return new IButtonRenderer<T>() {
            ...
            public void renderButton(Context context, String title, boolean focus, T state) {
                ...
                // Determine if the button is active based on the Boolean state
                boolean active = type == Boolean.class && (Boolean)state;
                Color backgroundColor = active ? new Color(0, 255, 0) : getBackgroundColor(effFocus); // Green if active, original color if not
                ...
                // Rest of the rendering code, which draws text and handles hover state
                int colorLevel = active ? 2 : 1; // Adjust color level based on active state
                if (context.isHovered()) {
                ...
                // Render the title of the button
                int xpos = context.getPos().x + padding;
                ...
                // Render additional state or value next to the button title if necessary
                // (e.g., ON/OFF for a toggle button)
                if (type == String.class) {
                    // ... (rendering code for String type state)
                } else if (type == Boolean.class) {
                    // ... (rendering code for Boolean type state)
                    // For instance, drawing the "ON"/"OFF" text
                    // This part may need adjustments depending on your design requirements
                }

                // Render the border of the button if needed
                if (!container) {
                    Color borderColor = graphicalLevel <= 0 ? scheme.getColor("Panel Outline Color") : scheme.getColor("Component Outline Color");
                    ITheme.drawRect(context.getInterface(), context.getRect(), borderColor);
                }
            }
    ...
    public IColorPickerRenderer getColorPickerRenderer() {
        return new StandardColorPicker() {
            ...
            public int getBaseHeight() {
                return org.eclipse.eclipse.theme.EclipseTheme.this.getBaseHeight();
            }
    ...
    @Override
    public Color getMainColor(boolean focus, boolean active) {
        if (active) return new Color(37, 150, 190); // Green for active/enabled
        return scheme.getColor("Disabled Color"); // Original color when not active/disabled
    }
