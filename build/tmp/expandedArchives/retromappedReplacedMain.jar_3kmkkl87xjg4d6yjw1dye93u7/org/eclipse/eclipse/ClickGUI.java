...
import org.eclipse.eclipse.theme.EclipseTheme;
import org.lwjgl.input.Keyboard;

import org.eclipse.eclipse.module.Category;
import org.eclipse.eclipse.module.ClickGUIModule;
import org.eclipse.eclipse.module.ClickGUIModule.Theme;
...
public class ClickGUI extends MinecraftHUDGUI {
	...
	public ClickGUI() {
		...
		ILayout horizontalCSGOLayout=new CSGOLayout(new Labeled("Example",null,()->true),new Point(100,100),480,WIDTH,animation,"Enabled",true,true,2,ChildMode.POPUP,colorPopup) {
			@Override
			public int getScrollHeight (Context context, int componentHeight) {
				return resizableHeight.apply(horizontalResizable.get()).getScrollHeight(null,field_146295_m);
			}
		};
		...
		ILayout verticalCSGOLayout=new CSGOLayout(new Labeled("Eclipse",null,()->true),new Point(100,100),480,WIDTH,animation,"Enabled",false,true,2,ChildMode.POPUP,colorPopup) {
			@Override
			public int getScrollHeight (Context context, int componentHeight) {
				return resizableHeight.apply(verticalResizable.get()).getScrollHeight(null,field_146295_m);
			}
		};
		...
		ILayout categoryCSGOLayout=new CSGOLayout(new Labeled("Example",null,()->true),new Point(100,100),480,WIDTH,animation,"Enabled",false,false,2,ChildMode.POPUP,colorPopup) {
			@Override
			public int getScrollHeight (Context context, int componentHeight) {
				return resizableHeight.apply(categoryResizable.get()).getScrollHeight(null,field_146295_m);
			}
		};
		...
		ILayout searchableCSGOLayout=new SearchableLayout(new Labeled("Example",null,()->true),new Labeled("Search",null,()->true),new Point(100,100),480,WIDTH,animation,"Enabled",2,ChildMode.POPUP,colorPopup,(a,b)->a.getDisplayName().compareTo(b.getDisplayName()),charFilter,keys) {
			@Override
			public int getScrollHeight (Context context, int componentHeight) {
				return resizableHeight.apply(searchableResizable.get()).getScrollHeight(null,field_146295_m);
			}
		};
	...
	private class ThemeSelector implements IThemeMultiplexer {
		...
		public ThemeSelector (IInterface inter) {
			...
			addTheme(Theme.EclipseTheme, new EclipseTheme(new ThemeScheme(Theme.EclipseTheme),9,4));
